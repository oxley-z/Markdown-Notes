...
* **路由控制**：根据中断类型，GIC需决定由哪个PE接收中断。
	...
	* 对于LPIs，如果实现了ITS，则路由信息来自于ITS。
	* PPIs特定与一个PE，并且只能由该PE处理。
	* 对于SGIs，发起中断的PE定义了目标PE列表。
* **中断优先级&优先级掩码**：每个PE在CPU interface层面都有一个优先级掩码寄存器（ICC_PMR_EL1），该寄存器设置中断转发到该PE所需的最低优先级，只有比该优先级高的中断才能被发送到该PE。
* **运行时优先级**：如果PE未处理中断，则优先级为（0xFF），如果PE在处理中断，只有优先级高于正在运行的中断的优先级，才能抢占当前运行的中断。

#### 中断响应

CPU interface有两个IAR（Interrupt Acknowledge Register）寄存器，读取IAR寄存器会返回对应的中断ID，并推进中断状态机，在典型的中断处理程序中，处理中断的首要步骤就是读取其中的IAR
...
伪中断是指：CPU 进入了中断异常处理流程，但读取 IAR 时发现**没有有效的 pending 中断可以处理**。此时 IAR 返回的是一个**特殊的保留 INTID**，而不是真实的中断号。

| 中断号  | 含义                                                                          | 示例场景                                                                           |
| ---- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| 1020 | 仅由ICC_IAR0_EL1的读取返回；<br/>最高待处理中断为Non-secure Group 1；<br/>仅在将FIQ升级到EL3时才会出现； | 当PE处于Non-secure状态时，可信操作系统收到中断信号，这被视为 EL3 的 FIQ，以便安全监视器可以切换到可信操作系统（以FIQ陷入到EL3）。 |
| 1021 | 仅由ICC_IAR0_EL1的读取返回；<br/>最高待处理中断为Non-secure Group 1；<br/>仅在将FIQ升级到EL3时才会出现； | 当PE处于Secure状态时，向rich操作系统发出了中断信号。这将被视为 EL3 的 FIQ，以便安全监视器可以切换到rich操作系统。          |
| 1022 | 用于旧版本操作                                                                     |                                                                                |
| 1023 | 伪中断<br/>Pending状态下没有已启用的INTID，或者所有Pending状态的INTID的优先级都不足以被执行；               | 此值表示没有中断可供响应。                                                                  |
