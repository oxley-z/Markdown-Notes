...
- 此基准测试工具从CPU周期计数器获取时间戳，某些CPU架构可能不具备这种功能；
- 时延基准测试测量的是（数据包的）往返时间，其中一半可看作单向的时延，这意味着对于不对称的物理配置（两个方向时延不同），结果可能不准确；
- 所有的单项带宽基准测试中，在客户端测量带宽；
- 在双向带宽基准测试中，谁发起流量，谁就测试带宽，在一个测试周期结束时，服务端将结果告知客户端，再有客户端将他们组合在一起；
- 时延测试会报告最大、最小以及中值，与平均时延测试相比，中值时延通常对于高延时的变化不太敏感。通常由于首次启动的原因，测试的第一个值是最大值；
- 长采样周期对测量精度的影响非常有限。程序默认进行 1000 次迭代，这个默认值以较为合适。另外请注意，程序保存测试结果的数据结构的内存占用量与迭代次数成正比。设置非常多的迭代次数可能会对测量的性能结果产生负面影响，这种对性能的影响与被测设备无关。如果一定需要大量迭代，建议使用 -N 选项（无峰值）。
- 带宽基准测试可以运行多次迭代，也可以运行固定的持续时间。使用 -D 选项指示测试程序运行指定的秒数。使用 --run—infinitely 选项指示程序持续运行直到被用户中断，并每隔 5 秒打印测量的带宽。
