...
| 14:12 | **Undefined**                                            | 从这些位读取的值未定义。在之前的版本中，根据规范，此位用于指示适配器上是否实现了注意按钮、注意指示灯或电源指示灯，并由适配器上的组件进行电气控制。系统软件必须忽略从此位读取的值。<br>系统软件可以向该位写入任何值。                                                                                                                                                                                                                                                                                                                                                                                        |     |
| 15    | **Role-Based Error Reporting**                           | 此位设置后，表示该Func实现了最初在 \[PCIe-1.0a\] 错误报告 ECN 中定义的功能，该功能后来被纳入 \[PCIe-1.1\]。所有符合 ECN、\[PCIe-1.1\] 或后续 \[PCIe\] 修订版的功能都必须设置此位。                                                                                                                                                                                                                                                                                                                                                                           |     |
| 16    | **ERR_COR Subclass Capable**                             | 设置此位后，表示该Func支持 ERR_COR 消息中的 ERR_COR 子类字段，从而可以区分不同的子类。<br/>实现了系统固件中间件 (SFI) 功能的下游端口必须设置此位。强烈建议实现了下游端口隔离 (DPC) 功能的下游端口也设置此位。                                                                                                                                                                                                                                                                                                                                                                         |     |
| 25:18 | **Captured Slot Power Limit Value**(Upstream Ports only) | 与捕获插槽功率限制比例值，指定适配器可用功率的上限。<br>功率限制（单位：瓦特）的计算方法是将此字段中的值乘以“捕获插槽功率限制比例”字段中的值，但“捕获插槽功率限制比例”字段除外。<br/><br>如果字段等于 00b (1.0x) 且捕获的插槽功率限制值超过 EFh，则使用替代编码。<br/><br>该值由 Set_Slot_Power_Limit 消息设置，或硬编码为 00h。默认值为 00h。                                                                                                                                                                                                                                                                                         |     |
